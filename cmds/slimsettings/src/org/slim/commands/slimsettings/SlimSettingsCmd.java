...
package org.slim.commands.slimsettings;
...
public final class SlimSettingsCmd {
            ...
            try {
                ...
                try {
                    ContentProviderHolder holder = activityManager.getContentProviderExternal(
                            "slimsettings", UserHandle.USER_SYSTEM, token);
                    if (holder == null) {
    ...
    private List<String> listForUser(IContentProvider provider, int userHandle, String table) {
        final Uri uri = "system".equals(table) ? SlimSettings.System.CONTENT_URI
                : "secure".equals(table) ? SlimSettings.Secure.CONTENT_URI
                : "global".equals(table) ? SlimSettings.Global.CONTENT_URI
                : null;
    ...
    String getForUser(IContentProvider provider, int userHandle,
        ...
        if ("system".equals(table)) callGetCommand = SlimSettings.CALL_METHOD_GET_SYSTEM;
        else if ("secure".equals(table)) callGetCommand = SlimSettings.CALL_METHOD_GET_SECURE;
        else if ("global".equals(table)) callGetCommand = SlimSettings.CALL_METHOD_GET_GLOBAL;
        else {
        ...
        try {
            ...
            arg.putInt(SlimSettings.CALL_METHOD_USER_KEY, userHandle);
    ...
    void putForUser(IContentProvider provider, int userHandle,
        ...
        if ("system".equals(table)) callPutCommand = SlimSettings.CALL_METHOD_PUT_SYSTEM;
        else if ("secure".equals(table)) callPutCommand = SlimSettings.CALL_METHOD_PUT_SECURE;
        else if ("global".equals(table)) callPutCommand = SlimSettings.CALL_METHOD_PUT_GLOBAL;
        else {
            System.err.println("Invalid table; no put performed");
            return;
        }
    ...
    int deleteForUser(IContentProvider provider, int userHandle,
            final String table, final String key) {
        Uri targetUri;
        if ("system".equals(table)) targetUri = SlimSettings.System.getUriFor(key);
        else if ("secure".equals(table)) targetUri = SlimSettings.Secure.getUriFor(key);
        else if ("global".equals(table)) targetUri = SlimSettings.Global.getUriFor(key);
        else {
    ...
    private static void printUsage() {
        System.err.println("usage:  slimsettings [--user <USER_ID> | current] get namespace key");
        System.err.println("        slimsettings [--user <USER_ID> | current] put namespace key value");
        System.err.println("        slimsettings [--user <USER_ID> | current] delete namespace key");
        System.err.println("        slimsettings [--user <USER_ID> | current] list namespace");
