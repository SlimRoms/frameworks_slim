...
import org.slim.providers.settings.SettingsState.Setting;
...
public class SlimSettingsProvider extends ContentProvider {
    ...
    static {
        SlimSettings.Secure.getMovedToGlobalSettings(sSecureMovedToGlobalSettings);
    }
    ...
    static {
        SlimSettings.System.getMovedToGlobalSettings(sSystemMovedToGlobalSettings);
    }
    ...
    static {
        SlimSettings.System.getMovedToSecureSettings(sSystemMovedToSecureSettings);
    }
    ...
    static {
        SlimSettings.Global.getMovedToSecureSettings(sGlobalMovedToSecureSettings);
    }
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        String name = values.getAsString(SlimSettings.Secure.NAME);
        if (!isKeyValid(name)) {
            return null;
        }

        String value = values.getAsString(SlimSettings.Secure.VALUE);

        switch (table) {
            case TABLE_GLOBAL: {
                if (insertGlobalSetting(name, value, UserHandle.getCallingUserId(), false)) {
                    return Uri.withAppendedPath(SlimSettings.Global.CONTENT_URI, name);
                }
            } break;

            case TABLE_SECURE: {
                if (insertSecureSetting(name, value, UserHandle.getCallingUserId(), false)) {
                    return Uri.withAppendedPath(SlimSettings.Secure.CONTENT_URI, name);
                }
            } break;

            case TABLE_SYSTEM: {
                if (insertSystemSetting(name, value, UserHandle.getCallingUserId())) {
                    return Uri.withAppendedPath(SlimSettings.System.CONTENT_URI, name);
                }
            } break;
    ...
    private int resolveOwningUserIdForSecureSettingLocked(int userId, String setting) {
        return resolveOwningUserIdLocked(userId, setting);
    }
    ...
    private int resolveOwningUserIdLocked(int userId, String name) {
        return userId;
    }
    ...
    private static int getRequestingUserId(Bundle args) {
        ...
        return (args != null) ? args.getInt(SlimSettings.CALL_METHOD_USER_KEY, callingUserId)
                : callingUserId;
    }

    private boolean isTrackingGeneration(Bundle args) {
        return args != null && args.containsKey(SlimSettings.CALL_METHOD_TRACK_GENERATION_KEY);
    }
    ...
    private static String getValidTableOrThrow(Uri uri) {
        if (uri.getPathSegments().size() > 0) {
            ...
            if (SlimDatabaseHelper.isValidTable(table)) {
                return table;
            }
    ...
    final class SettingsRegistry {
        ...
        private static final String SETTINGS_FILE_GLOBAL = "slim_settings_global.xml";
        private static final String SETTINGS_FILE_SYSTEM = "slim_settings_system.xml";
        private static final String SETTINGS_FILE_SECURE = "slim_settings_secure.xml";
            ...
            synchronized (mLock) {
                ...
                try {
                    ...
                    for (int i = 0; i < userCount; i++) {
                        ...
                        SlimDatabaseHelper dbHelper = new SlimDatabaseHelper(getContext(), userId);
        ...
        private void migrateLegacySettingsForUserLocked(SlimDatabaseHelper dbHelper,
        ...
        private Uri getNotificationUriFor(int key, String name) {
            if (isGlobalSettingsKey(key)) {
                return (name != null) ? Uri.withAppendedPath(SlimSettings.Global.CONTENT_URI, name)
                        : SlimSettings.Global.CONTENT_URI;
            } else if (isSecureSettingsKey(key)) {
                return (name != null) ? Uri.withAppendedPath(SlimSettings.Secure.CONTENT_URI, name)
                        : SlimSettings.Secure.CONTENT_URI;
            } else if (isSystemSettingsKey(key)) {
                return (name != null) ? Uri.withAppendedPath(SlimSettings.System.CONTENT_URI, name)
                        : SlimSettings.System.CONTENT_URI;
            } else {
                throw new IllegalArgumentException("Invalid settings key:" + key);
            }
        }
        ...
        private final class UpgradeController {
            ...
            public void upgradeIfNeededLocked() {
                ...
                if (curVersion != newVersion) {
                    ...
                    SlimDatabaseHelper dbHelper = new SlimDatabaseHelper(getContext(), mUserId);
                    SQLiteDatabase database = dbHelper.getWritableDatabase();
