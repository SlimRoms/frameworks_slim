                    ...
                    if (index >= 0) {
                        ...
                        if (DEBUG) {
                            Slog.i(LOG_TAG, "Exported index:" + index + " for key:"
                                    + SlimSettingsProvider.keyToString(key));
                        }
                    }
                }
            } catch (IOException e) {
    ...
    public void onUserRemoved(int userId) {
        synchronized (mLock) {
            ...
            if (backingStore != null && mKeyToIndexMap.size() > 0) {
                try {
                    final int secureKey = SlimSettingsProvider.makeKey(
                            SlimSettingsProvider.SETTINGS_TYPE_SECURE, userId);
                    resetSlotForKeyLocked(secureKey, mKeyToIndexMap, backingStore);

                    final int systemKey = SlimSettingsProvider.makeKey(
                            SlimSettingsProvider.SETTINGS_TYPE_SYSTEM, userId);
