...
public class SlimLightsService extends SlimSystemService {
    ...
    private boolean mNotificationLedScreenOnEnabled = false;
    ...
    class SettingsObserver extends ContentObserver {
        ...
        protected void observe() {
            ...
            resolver.registerContentObserver(SlimSettings.System.getUriFor(
                    ...
                    false, this, UserHandle.USER_ALL);
            resolver.registerContentObserver(SlimSettings.System.getUriFor(
                    SlimSettings.System.NOTIFICATION_LIGHT_SCREEN_ON),
                    false, this, UserHandle.USER_ALL);
        ...
        protected void update() {
            ...
            mNotificationLedScreenOnEnabled = SlimSettings.System.getIntForUser(resolver,
                    SlimSettings.System.NOTIFICATION_LIGHT_SCREEN_ON, 0,
                    UserHandle.USER_CURRENT) != 0;
    ...
    private void handleNotificationPosted(StatusBarNotification sbn,
        ...
        boolean aboveThreshold = sbn.getNotification().extras.getBoolean("force_lights", false);
        for (int i = 0; i < nru.getOrderedKeys().length; i++) {
            if (nru.getOrderedKeys()[i].equals(key)) {
                aboveThreshold = nru.getImportance()[i] > IMPORTANCE_DEFAULT || aboveThreshold;
                break;
            }
        }
    ...
    if (DEBUG) Log.d(TAG, "updateLightsLocked");
        synchronized (mLock) {
            ...
            if (sbn == null) {
                enableLed = false;
            } else if (sbn.getNotification().extras.getBoolean("force_lights", false)) {
                enableLed = true;
            } else if (mInCall) {
                enableLed = false;
            } else if (!mNotificationLedScreenOnEnabled && mScreenOn) {
                enableLed = false;
            } else {
                enableLed = true;
            }
            if (DEBUG) Log.d(TAG, "enableLed - " + enableLed + " : mScreenOn - " + mScreenOn);
            ...
            if (!enableLed) {
                ...
                if (DEBUG) Log.d(TAG, "ledARGB - " + ledARGB + " : ledOnMS - "
                        + ledOnMS + " : ledOffMS - " + ledOffMS);
